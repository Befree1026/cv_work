...
# 预处理，设置了一堆A用来数据增强 + 加载数据然后不符合的给resize一下
# 函数里面的参数有值应该是为了self的时候方便传过去，不是就固定这个值了x
class MyDataset(Dataset):
    ...
    def load_item(self, idx):
        ...
        if img.shape != (896,  896,  3):
            # img.resize((self.image_size, self.image_size, img.shape[2]))
            # mask.resize((self.image_size, self.image_size, mask.shape[2]))
            # img.resize((self.image_size, self.image_size))
            # mask.resize((self.image_size, self.image_size))
            # img = np.reshape(img, (self.image_size, self.image_size, img.shape[2]))
            # mask = np.reshape(mask, (self.image_size, self.image_size, mask.shape[2]))
...
# The network used for OSN noise modeling  用于建模OSN噪声的网络
...
class Detector(nn.Module):
    ...
    def __init__(self):
        ...
        try:
            self.osn_net.load_state_dict(torch.load('D:\python object\ImageForensicsOSN-main\weights\OSN_Unet_weights.pth'))
        except:
...
class ForgeryForensics():
    def __init__(self):
        self.batch_size = 1  # modify according to the resources
